...
extension CamView: AVCapturePhotoCaptureDelegate {
   ...
   @objc open func photoOutput(_ output: AVCapturePhotoOutput, didFinishProcessingPhoto photo: AVCapturePhoto, error: Error?) {
      ...
      if let capturedImage = UIImage(data: imageData, scale: 1.0) { // Initialise an UIImage with our image data
         guard let fileURL: URL = FileUtil.tempURL(suffix: ".jpeg") else { onPhotoCaptureComplete(.failure(.unableToCreateTempFilePath)); return }
         guard !FileManager().fileExists(atPath: fileURL.path) else { onPhotoCaptureComplete(.failure(.filePathAlreadyExists)); return } // This will never happen because it uses a very random file name
